...
function Dashboard() {
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen p-6 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'}`}>
      <header className={`header-container w-full bg-white shadow-md p-4 rounded-md flex flex-col md:flex-row items-center justify-between mt-0 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
        <h1 className={`header-title text-4xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Header</h1>
        ...
        <div className="profile-container flex items-center">
          ...
          <span className={`text-gray-800 font-semibold ml-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{username}</span>
        </div>
      </header>

      {/* Profile Settings Container */}
      {isProfileSettingsOpen && (
        <div className={`profile-settings-container mt-4 rounded-lg p-6 shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          <div className="flex flex-col mb-4">
            ...
            <input
              ...
              className={`border border-gray-300 rounded-md p-2 ${isDarkMode ? 'bg-gray-700 text-white' : ''}`}
            />
          </div>
          ...
          <div className="flex items-center mb-4">
            ...
            <button onClick={toggleMode} className="p-2 border rounded-md bg-gray-200 hover:bg-gray-300">
          ...
          <button onClick={() => setIsProfileSettingsOpen(false)} className="mt-4 bg-blue-500 text-white p-2 rounded-md">Save</button>
        </div>
      )}
    </div>
  );
